...
export default function AboutClient({ initialTeam }: { initialTeam: TeamMember[] }) {
  ...
  useEffect(() => {
    // We already have initialTeam from the server, but we can fetch updates if needed
    // In this case, we just rely on the server data for instant loading
  }, []);
        ...
        <Container className="relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
            {team?.map((member, index) => (
              ...
              >
                {/* Image / Placeholder */}
                <div className="h-72 md:h-80 bg-gradient-to-b from-[#111827] to-[#0a0f1d] relative flex items-center justify-center overflow-hidden">
                  {member.image_url ? (
